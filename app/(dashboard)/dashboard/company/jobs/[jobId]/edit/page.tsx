...
export default function EditJobPosting() {
  ...
  useEffect(() => {
    ...
    const fetchJobData = async () => {
      setIsFetching(true);
      try {
        const response = await fetch(`/api/jobs/company/${jobId}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (publish: boolean = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push("/dashboard/company/jobs")
        router.refresh(); // To ensure the jobs list is updated
      } else {
        const errorData = await response.json();
        toast.error(errorData.message || "Failed to update job posting.")
      }
    } catch (error) {
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2 mb-4 text-sm">
                        ...
                        {formData.createdAt && (
                          <Badge variant="outline" className="font-normal text-xs border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200">
                            <Calendar className="w-3.5 h-3.5 mr-1" />Posted: {new Date(formData.createdAt).toLocaleDateString()}
                          </Badge>
                        )}
                      </div>
                  </div>
                  ...
                  {formData.description && (
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground whitespace-pre-wrap break-words leading-relaxed line-clamp-4">{formData.description}</p>
                    </div>
                  )}
                    ...
                    <div className="pt-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.slice(0, 6).map((skill) => (
                          <Badge key={skill.name} variant="secondary" className="px-2 py-1 bg-primary/10 text-primary border-primary/20">
                            <span>{skill.name}</span><span className="text-xs opacity-60 ml-1.5 capitalize">({skill.level.toLowerCase()})</span>
                          </Badge>
                        ))}
                        {skills.length > 6 && <Badge variant="outline" className="text-xs">+{skills.length - 6} more</Badge>}
                      </div>
                    </div>
                  )}
                </CardContent>
